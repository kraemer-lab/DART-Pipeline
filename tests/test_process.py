...
@patch("rasterio.open")
@patch("dart_pipeline.process.get_chirps_rainfall_data_path")
def test_process_chirps_rainfall(mock_data_path, mock_raster_open, mock_output_path):
    partial_date = "2023-05"
    plots = False
...
def test_process_terraclimate(mock_source_path, mock_read_file, mock_nc_dataset):
    ...
    mock_nc.variables = {
        ...
        "time": [0, 31, 59],
        "aet": MagicMock(
            __getitem__=MagicMock(return_value=np.array([[[0.5, 0.6], [0.7, 0.8]]])),
            scale_factor=1.0,
            add_offset=0.0,
            _FillValue=-9999.0,
