...
def test_plot_heatmap(mock_subplots, mock_colorbar, mock_savefig):
...
@patch("matplotlib.pyplot.colorbar")
@patch("matplotlib.pyplot.savefig")
def test_plot_gadm_macro_heatmap(mock_savefig, mock_colorbar, mock_subplots, tmp_path):
...
def test_plot_gadm_scatter(mock_mkdir, mock_savefig, mock_close):
    ...
    # Mock GeoDataFrame
    gdf = gpd.GeoDataFrame({"geometry": [box(5, 35, 35, 65)]})
