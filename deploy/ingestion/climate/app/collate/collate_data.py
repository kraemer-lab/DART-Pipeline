...
def get_credentials(metric, base_dir="..", credentials=None):
    ...
    try:
        ...
        msg = (
            "No credentials.json file was found. Either you have not "
    ...
    if metric not in credentials.keys():
        msg = (
            f'No credentials for "{metric}" exists in the credentials '
            + f'file "{path}"'
        )
        raise KeyError(msg)
...
def download_aphrodite_temperature_data(
    ...
    **Requires APHRODITE account**
    ...
    for key in relative_urls.keys():
        relative_url = relative_urls[key]
        walk(base_url, relative_url, only_one, dry_run, out_dir, username, password)
...
def download_worldpop_pop_density_data(only_one, dry_run):
    ...
    # Construct URL
    relative_url = (
        "GIS/Population_Density/Global_2000_2020_1km_UNadj/"
...
def process(data_name, only_one=False, dry_run=False, credentials=None, year=None):
    # Print arguments
    print("Arguments:")
    print(f"data_name: {data_name}")
    print(f"only_one: {only_one}")
    ...
    if data_name == "":
        ...
        download_meteorological_data(data_name, only_one, dry_run, credentials, year)
...
if __name__ == "__main__":
    ...
    process(data_name, only_one, dry_run, credentials, year)
