...
def process_chirps_rainfall_data(year, debug):
    ...
    for filepath in filepaths:
        ...
        path = Path(
            ...
            + "Observations",
            Path(filepath.name).with_suffix(".csv"),
        )
        path.parent.mkdir(parents=True, exist_ok=True)
        ...
        if False:
            ...
            path = Path(
                ...
                "CHIRPS - Rainfall Estimates from Rain Gauge and Satellite "
                + "Observations",
                Path(filepath.name).with_suffix(".png"),
            )
            plt.savefig(path)
...
def process_era5_reanalysis_data():
    ...
    path = Path(
        ...
        "ERA5-ml-temperature-subarea.nc",
    )
    ...
    path = Path(
        ...
        "Meteorological Data",
        "ERA5 atmospheric reanalysis",
        "ERA5-ml-temperature-subarea.csv",
    )
    path.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(path, index=False)
    ...
    for year in [year]:
        for metric in metrics:
            ...
            for month in [month]:
                ...
                figsize = (46.82 * 0.5 ** (0.5 * A), 33.11 * 0.5 ** (0.5 * A))
...
def process_worldpop_pop_density_data(year, country_iso3):
    ...
    figsize = (33.11 * 0.5 ** (0.5 * A), 46.82 * 0.5 ** (0.5 * A))
...
def process_geospatial_meteorological_data(data_name, admin_level, iso3, year):
...
def process_geospatial_sociodemographic_data(data_name, admin_level, iso3, year, rt):
...
shorthand_to_data_name = {
    # Meteorological Data
    "APHRODITE precipitation": "APHRODITE Daily accumulated precipitation (V1901)",
    "APHRODITE temperature": "APHRODITE Daily mean temperature product (V1808)",
    "CHIRPS rainfall": "CHIRPS: Rainfall Estimates from Rain Gauge and Satellite Observations",
    "TerraClimate data": "TerraClimate gridded temperature, precipitation, and other",
    "ERA5 reanalysis": "ERA5 atmospheric reanalysis",
    # Socio-Demographic Data
...
data_name_to_type = {
    ...
    "CHIRPS: Rainfall Estimates from Rain Gauge and Satellite Observations": "Meteorological Data",
    "TerraClimate gridded temperature, precipitation, and other": "Meteorological Data",
...
def process(
    data_name, admin_level=None, iso3=None, year=None, month=None, rt=None, debug=False
):
    # print arguments
    if debug:
        print("Arguments:")
    ...
    #     process_epidemiological_data(data_name[0], iso3, admin_level)
        ...
        process_geospatial_meteorological_data(data_name, admin_level, iso3, year)
    elif data_type == ["Geospatial Data", "Socio-Demographic Data"]:
        process_geospatial_sociodemographic_data(data_name, admin_level, iso3, year, rt)
...
if __name__ == "__main__":
    ...
    process(data_name, admin_level, iso3, year, month, rt, debug)
