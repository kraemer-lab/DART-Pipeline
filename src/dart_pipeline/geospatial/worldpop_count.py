...
from dart_pipeline.util import source_path, get_shapefile, get_country_name, output_path
...
def process_gadm_worldpopcount(
    iso3, partial_date: str, admin_level: AdminLevel = "0", rt: str = "ppp", plots=False
):
    ...
    if pdate.day:
        msg = (
            f"The date {partial_date} includes a day. Provide only a "
            + "year in YYYY format."
        )
        raise ValueError(msg)
    if pdate.month:
        msg = (
            f"The date {partial_date} includes a month. Provide only a "
            + "year in YYYY format."
        )
        raise ValueError(msg)
    ...
    try:
        ...
        if not file_found:
            msg = (
                f"{path}: No such file or directory. Either it has not "
